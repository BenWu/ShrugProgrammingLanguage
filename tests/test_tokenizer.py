...
from shrug_lang.tokenizer import Tokenizer, Tokens
...
class TestLineParser(unittest.TestCase):
    ...
    def test_empty_line(self):
        ...
        self.assertEqual(empty_line, self.parser.parse_line(line))
    ...
    def test_shrug(self):
        ...
        self.assertEqual(single_shrug, self.parser.parse_line(line))

    def test_numbers(self):
        line = '2'
        single_number1 = [(Tokens.NUMBER, 2), (Tokens.EOL,)]
        self.assertEqual(single_number1, self.parser.parse_line(line))

        line = '43435356'
        single_number2 = [(Tokens.NUMBER, 43435356), (Tokens.EOL,)]
        self.assertEqual(single_number2, self.parser.parse_line(line))

        line = 'dfs3'
        invalid_id1 = [(Tokens.INVALID, 'dfs3'), (Tokens.EOL,)]
        self.assertEqual(invalid_id1, self.parser.parse_line(line))

        line = 'abc%'
        invalid_id2 = [(Tokens.INVALID, 'abc%'), (Tokens.EOL,)]
        self.assertEqual(invalid_id2, self.parser.parse_line(line))

    def test_ids(self):
        line = 'a'
        single_id1 = [(Tokens.ID, 'a'), (Tokens.EOL,)]
        self.assertEqual(single_id1, self.parser.parse_line(line))

        line = 'variable'
        single_id2 = [(Tokens.ID, 'variable'), (Tokens.EOL,)]
        self.assertEqual(single_id2, self.parser.parse_line(line))

    def test_strings(self):
        line = '""'
        empty_string = [(Tokens.STRING, ''), (Tokens.EOL,)]
        self.assertEqual(empty_string, self.parser.parse_line(line))

        line = '"a"'
        short_string = [(Tokens.STRING, 'a'), (Tokens.EOL,)]
        self.assertEqual(short_string, self.parser.parse_line(line))
        ...
        self.assertEqual(long_string, self.parser.parse_line(line))
        ...
        self.assertEqual(weird_string, self.parser.parse_line(line))

        line = '"a'
        invalid_string = [(Tokens.INVALID, '"a'), (Tokens.EOL,)]
        self.assertEqual(invalid_string, self.parser.parse_line(line))

        line = '"¯\_(ツ)_/¯"'
        shrug_string = [(Tokens.STRING, '¯\_(ツ)_/¯'), (Tokens.EOL,)]
        self.assertEqual(shrug_string, self.parser.parse_line(line))
    ...
    def test_complex(self):
        ...
        self.assertEqual(tokens, self.parser.parse_line(line))
